...
Use `tak_cli` for all tak lifecycle/coordination calls. Use `bash` for non-tak commands only.
...
After all planned tasks complete:
...
Stop and ask immediately if instructions are unclear, verification fails repeatedly, or dependencies block progress. Do not guess and do not bypass tak state updates.
